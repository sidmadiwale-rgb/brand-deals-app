...
def campaign_detail_dialog(camp_dict):
    ...
    for i, p in enumerate(posts):
        ...
        ig_comments = _safe_int(p.get("IG Comments"))
        platforms = []
        if p.get("IG URL") or ig_v: platforms.append("<span style='color:#1877F2;font-weight:500'>IG</span>")
        if p.get("YT URL") or yt_v: platforms.append("<span style='color:#E24B4A;font-weight:500'>YT</span>")
        if p.get("TT URL") or tt_v: platforms.append("<span style='color:#FAFAFA;font-weight:500'>TT</span>")
        platforms_str = " · ".join(platforms) if platforms else "—"
        st.markdown(f"<div style='background:#18181B;border:1px solid #27272A;border-radius:10px;padding:10px;margin-bottom:6px'>"
                    f"<div style='display:flex;justify-content:space-between;align-items:start'>"
                    f"<div style='font-size:11px;color:#A1A1AA'>{platforms_str}</div>"
                    f"<div style='font-size:10px;color:#A1A1AA'>{d_date}</div></div>"
                    f"<p style='font-size:11px;color:#FAFAFA;margin:4px 0 6px;opacity:0.85'>{d_caption}</p>"
                    ...
                    f"<div><div style='font-size:14px;font-weight:500;color:#FAFAFA'>{format_compact_int(ig_likes)}</div>"
                    f"<div style='font-size:9px;color:#A1A1AA;text-transform:uppercase'>Likes</div></div>"
                    f"<div><div style='font-size:14px;font-weight:500;color:#FAFAFA'>{format_compact_int(ig_comments)}</div>"
                    f"<div style='font-size:9px;color:#A1A1AA;text-transform:uppercase'>Comments</div></div></div></div>",
                    unsafe_allow_html=True)
        post_sr = int(p.get("_sheet_row", 0))
        del_conf_key = f"cd_post_del_{post_sr}"
        if st.session_state.get(del_conf_key):
            colA, colB = st.columns(2)
            with colA:
                if st.button("Confirm delete", key=f"cd_postcdel_{post_sr}", use_container_width=True):
                    try:
                        gc = get_gspread_client()
                        ws = gc.open_by_key(SHEET_ID).worksheet("Post Tracking")
                        ws.delete_rows(post_sr)
                        st.session_state[del_conf_key] = False
                        st.success("Post deleted."); st.cache_data.clear(); st.rerun()
                    except Exception as e: st.error(f"Delete failed: {e}")
            with colB:
                if st.button("Cancel", key=f"cd_postccancel_{post_sr}", use_container_width=True):
                    st.session_state[del_conf_key] = False; st.rerun()
        else:
            if st.button("Delete post", key=f"cd_postdel_{post_sr}", use_container_width=True):
                st.session_state[del_conf_key] = True; st.rerun()
        ...
        for item in filtered_c:
            ...
            if label:
                rating_chip = (f"<span style='background:{rb_bg};color:{rating_colors.get(label,'#A1A1AA')};"
                               f"font-size:10px;padding:2px 6px;border-radius:5px;font-weight:500'>{emoji}</span>")
            cpv_html = f"<div style='font-size:18px;font-weight:500;color:#E89E7E'>{format_cpv(cpv, currency)}</div>" if cpv > 0 else "<div style='font-size:14px;color:#6B6B73;font-style:italic'>No performance data yet</div>"
            ...
            st.markdown(f"<div class='deal-card' style='{border_left}'>"
...
with tabs[5]:
    ...
    if campaigns_raw.empty:
        ...
        if len(currencies_in_view) == 1:
            ...
            st.info("No campaigns with performance data in this filter.")
