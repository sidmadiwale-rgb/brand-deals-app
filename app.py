...
with tabs[4]:
    ...
    with st.form("add_deal", clear_on_submit=True):
        ...
        with c2:
            new_campaign = st.text_input("Campaign", placeholder="e.g. Plum #4, Etihad - Greece")
            new_brand = st.text_input("Brand", placeholder="Rolled-up (e.g. Plum) — blank = same as Campaign")
            new_parent = st.text_input("Parent/Group", placeholder="e.g. HUL, NMA — blank = same as Brand")
        ...
        if submitted:
            if not new_campaign:
                ...
                # Auto-fill Brand and Parent/Group if blank
                effective_brand = (new_brand or new_campaign).strip()
                effective_parent = (new_parent or effective_brand).strip()
                try:
                    gc = get_gspread_client()
                    ws = gc.open_by_key(SHEET_ID).worksheet("Deals Log")
                    headers = None
                    ...
                    if not headers:
                        ...
                        st.success(f"Added {new_campaign}!"); st.cache_data.clear()
                except Exception as e:
