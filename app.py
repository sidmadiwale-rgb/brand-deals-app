...
def make_hbar_chart(df, x_col, y_col, color="#FAFAFA", currency_symbol="$", precision=0):
    ...
    fig = go.Figure(data=[go.Bar(x=df[x_col], y=df[y_col], orientation="h",
        ...
        hovertemplate=f"<b>%{{y}}</b><br>{currency_symbol}%{{x:{fmt}}}<extra></extra>")])
    ...
    layout["xaxis"] = dict(showgrid=True, gridcolor="#27272A",
                            ...
                            tickprefix=currency_symbol, tickformat=fmt)
