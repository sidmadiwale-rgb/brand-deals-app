...
def parse_month_date(s):
    """Parse a date in many formats to a date object."""
    if not s:
        return None
    s = str(s).strip()
    # Try formats including 2-digit year
    for fmt in [
        "%m/%d/%y", "%m/%d/%Y",       # US: 11/01/25 or 11/01/2025
        "%d/%m/%y", "%d/%m/%Y",       # International: 01/11/25
        "%Y-%m-%d", "%d-%m-%Y", "%m-%d-%Y",
    ]:
        try:
...
def derive_month_date(month_name, fy):
    """Compute date from Month name + FY. Useful when Month Date column is missing/broken."""
    if not month_name or not fy:
        return None
    try:
        m = MONTHS.index(month_name) + 1
        fy_year = int(str(fy).replace("FY", ""))
        # FY26 = Jul 2025 - Jun 2026
        if m >= 7:  # July-December → year before
            year = 2000 + fy_year - 1
        else:       # January-June → FY year
            year = 2000 + fy_year
