...
st.set_page_config(page_title="Priya Sid Enterprise", page_icon="📊",
...
def render_global_header():
    ...
    st.markdown(f"<div class='period-pill {pill_class}' style='margin: 8px 0 2px'>📅 {period_label()}</div>",
                unsafe_allow_html=True)
    with st.expander("Change period / refresh data", expanded=False):
...
with tabs[1]:
    ...
    for d in filtered:
        ...
        st.markdown(f"<div class='deal-card'>"
            ...
            f"<div style='font-size:14px;font-weight:500'>{amt_str}</div>"
            f"<div style='margin-top:4px;display:flex;gap:4px;justify-content:flex-end;flex-wrap:wrap'>{pills}</div></div>"
            f"</div></div>", unsafe_allow_html=True)
        if st.button("View / Edit  ›", key=f"edit_btn_{sheet_row}", use_container_width=True):
            edit_deal_dialog(d)
...
with tabs[3]:
    ...
    for ar in sorted(ad_in_period, key=lambda x: x["md"], reverse=True):
        ...
        if has_data:
            st.markdown(f"<div class='deal-card'>"
                ...
                f"<div style='font-size:14px;font-weight:500'>{format_money(ar['total_aud'] * mult, display_cur)}</div></div></div>",
                unsafe_allow_html=True)
        else:
            st.markdown(f"<div class='deal-card' style='border-style:dashed;opacity:0.55'>"
                f"<div style='display:flex;justify-content:space-between;align-items:center'>"
                f"<div><div style='font-size:13px;font-weight:500'>{ar['month']}</div>"
    ...
    with st.expander("➕ Add this month's payout"):
        c1, c2 = st.columns(2)
        with c1:
            available_months = [ar["month"] for ar in ad_rev_monthly]
            new_month = st.selectbox("Month", available_months, key="ar_month")
            new_yt = st.number_input("YouTube (AUD)", min_value=0.0, step=10.0, key="ar_yt")
        with c2:
            new_fb_usd = st.number_input("Facebook (USD)", min_value=0.0, step=10.0, key="ar_fb")
        if st.button("Save Ad Revenue", key="ar_save", type="primary"):
            try:
                gc = get_gspread_client()
                ...
                for idx, row in enumerate(all_vals):
                    if len(row) > 0 and row[0] == new_month:
                        ...
                        st.success(f"Saved {new_month}!"); st.cache_data.clear(); st.rerun()
                        break
                else:
                    st.error(f"Month '{new_month}' not found")
            except Exception as e:
                st.error(f"Save failed: {e}")
