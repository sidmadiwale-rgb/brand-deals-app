...
# Fiscal-year month order (Jul → Jun) used by trend chart bucketing.
FY_MONTH_ORDER = ["July", "August", "September", "October", "November", "December",
...
[data-testid="stNumberInput"] > div > div{padding-right:8px!important}
...
/* Larger variant for taller cards (Campaigns + Posts in detail view) */
.card-overlay-trigger-lg{position:relative;z-index:1;cursor:pointer}
div[data-testid="stElementContainer"]:has(.card-overlay-trigger-lg)+div[data-testid="stElementContainer"] .stButton{margin-top:-114px;position:relative;z-index:5}
div[data-testid="stElementContainer"]:has(.card-overlay-trigger-lg)+div[data-testid="stElementContainer"] .stButton button{width:100%!important;min-height:110px!important;background:transparent!important;border:none!important;color:transparent!important;cursor:pointer!important;padding:0!important;font-size:0!important}
div[data-testid="stElementContainer"]:has(.card-overlay-trigger-lg)+div[data-testid="stElementContainer"] .stButton button:hover{background:rgba(255,255,255,0.03)!important;border:0.5px solid #3F3F46!important;border-radius:12px!important}
...
_CPV_TIER_COLORS = {"Incredible": "#97C459", "Great": "#C0DD97",
                    "Good": "#EF9F27", "High": "#E24B4A"}

def cpv_color(cpv_value, currency, fx_rates):
    """Return the hex color for a CPV value based on its rating tier."""
    if cpv_value is None or cpv_value <= 0: return "#FAFAFA"
    _, label = rate_cpv(cpv_value, currency, fx_rates)
    return _CPV_TIER_COLORS.get(label, "#FAFAFA")

def tile_grid_html(tiles):
    """Render a 2-col CSS grid of tiles.
    tiles: list of dicts with keys: label, value, color (optional hex), subtitle (optional html)"""
    parts = []
    for t in tiles:
        color = t.get("color") or "#FAFAFA"
        sub = t.get("subtitle", "")
        sub_html = (f"<div style='font-size:10px;color:#A1A1AA;margin-top:6px;line-height:1.3'>{sub}</div>"
                    if sub else "")
        parts.append(
            f"<div style='background:#18181B;border:1px solid #27272A;border-radius:12px;"
            f"padding:14px;min-height:90px;display:flex;flex-direction:column;justify-content:space-between'>"
            f"<div style='font-size:11px;color:#A1A1AA;text-transform:uppercase;letter-spacing:0.4px'>{t['label']}</div>"
            f"<div><div style='font-size:22px;font-weight:500;color:{color};line-height:1'>{t['value']}</div>"
            f"{sub_html}</div></div>"
        )
...
_CPV_LEGEND_HTML = (
    "<div style='font-size:10px;color:#71717A;margin-top:14px;line-height:1.6'>"
    "<span style='color:#97C459'>●</span> Incredible &lt;₹0.30 &nbsp; "
    "<span style='color:#C0DD97'>●</span> Great &lt;₹0.50 &nbsp; "
    "<span style='color:#EF9F27'>●</span> Good &lt;₹1.00 &nbsp; "
    "<span style='color:#E24B4A'>●</span> High ≥₹1.00"
    "</div>"
)
...
def make_hbar_chart(df, x_col, y_col, color="#FAFAFA", currency_symbol="$", precision=0, label_suffix="", color_map=None):
...
def edit_deal_dialog(deal):
    ...
    def _parse_date_dmy(s):
        ...
        return None
    with c1:
        cur_inv = _parse_date_dmy(deal.get("Invoice Date"))
        inv_obj = st.date_input("Invoice Date", value=cur_inv, key=f"e_idate_{sheet_row}", format="DD/MM/YYYY")
        new_inv_date = inv_obj.strftime("%d/%m/%Y") if inv_obj else ""
    with c2:
        cur_pay = _parse_date_dmy(deal.get("Payment Date"))
        pay_obj = st.date_input("Payment Date", value=cur_pay, key=f"e_pdate_{sheet_row}", format="DD/MM/YYYY")
        new_pay_date = pay_obj.strftime("%d/%m/%Y") if pay_obj else ""
...
def campaign_aggregates(campaign_row, posts_for_camp):
    ...
    yt_views = yt_likes = yt_comments = 0
    tt_views = tt_likes = tt_comments = tt_shares = tt_saves = 0
    for p in posts_for_camp:
        ...
        yt_likes    += _safe_int(p.get("YT Likes"))
        yt_comments += _safe_int(p.get("YT Comments"))
        tt_views    += _safe_int(p.get("TT Views"))
        tt_likes    += _safe_int(p.get("TT Likes"))
        tt_comments += _safe_int(p.get("TT Comments"))
        tt_shares   += _safe_int(p.get("TT Shares"))
        tt_saves    += _safe_int(p.get("TT Saves"))
    combined_views = ig_views + yt_views + tt_views
    combined_engagement = (ig_likes + ig_comments + ig_reposts + ig_shares + ig_saves
                           + yt_likes + yt_comments
                           + tt_likes + tt_comments + tt_shares + tt_saves)
...
def add_post_dialog(campaign_id, campaign_name):
    ...
    with st.form(f"add_post_form_{campaign_id}", clear_on_submit=False):
        ...
        c1, c2 = st.columns(2)
        with c1:
            d_date_obj = st.date_input("Date posted", value=date.today(), key=f"ap_date_{campaign_id}", format="YYYY-MM-DD")
            d_date = d_date_obj.strftime("%Y-%m-%d") if d_date_obj else ""
        with c2:
            d_title = st.text_input("Title / Caption", placeholder="Optional", key=f"ap_title_{campaign_id}")
        st.markdown("<div style='display:flex;align-items:center;gap:8px;margin-top:14px;margin-bottom:6px'>"
                    "<i class='ti ti-brand-instagram' style='color:#1877F2;font-size:22px'></i>"
                    "<span style='font-size:13px;font-weight:600;color:#FAFAFA'>Instagram</span>"
                    "<span style='font-size:11px;color:#A1A1AA'>(optional)</span></div>", unsafe_allow_html=True)
        ig_url = st.text_input("IG URL", placeholder="instagram.com/p/…", key=f"ap_ig_url_{campaign_id}", label_visibility="collapsed")
        ...
        st.markdown("<div style='display:flex;align-items:center;gap:8px;margin-top:14px;margin-bottom:6px'>"
                    "<i class='ti ti-brand-youtube' style='color:#E24B4A;font-size:22px'></i>"
                    "<span style='font-size:13px;font-weight:600;color:#FAFAFA'>YouTube</span>"
                    "<span style='font-size:11px;color:#A1A1AA'>(optional)</span></div>", unsafe_allow_html=True)
        yt_url = st.text_input("YT URL", placeholder="youtu.be/… or youtube.com/watch?v=…", key=f"ap_yt_url_{campaign_id}", label_visibility="collapsed")
        yc1, yc2, yc3 = st.columns(3)
        with yc1:
            yt_views = st.number_input("YT Views", min_value=0, step=100, key=f"ap_yt_v_{campaign_id}")
        with yc2:
            yt_likes = st.number_input("YT Likes", min_value=0, step=10, key=f"ap_yt_l_{campaign_id}")
        with yc3:
            yt_comments = st.number_input("YT Comments", min_value=0, step=1, key=f"ap_yt_c_{campaign_id}")
        st.markdown("<div style='display:flex;align-items:center;gap:8px;margin-top:14px;margin-bottom:6px'>"
                    "<i class='ti ti-brand-tiktok' style='color:#FAFAFA;font-size:22px'></i>"
                    "<span style='font-size:13px;font-weight:600;color:#FAFAFA'>TikTok</span>"
                    "<span style='font-size:11px;color:#A1A1AA'>(optional)</span></div>", unsafe_allow_html=True)
        tt_url = st.text_input("TT URL", placeholder="tiktok.com/@…/video/…", key=f"ap_tt_url_{campaign_id}", label_visibility="collapsed")
        ...
        with tc4: tt_shares = st.number_input("TT Shares", min_value=0, step=10, key=f"ap_tt_s_{campaign_id}")
        with tc5: tt_saves = st.number_input("TT Saves", min_value=0, step=10, key=f"ap_tt_sv_{campaign_id}")
        ...
        if submitted:
            try:
                ...
                fields = {"Post ID": pid, "Campaign ID": campaign_id, "Date": d_date,
                    ...
                    "YT URL": yt_url, "YT Views": yt_views or "",
                    "YT Likes": yt_likes or "", "YT Comments": yt_comments or "",
                    "TT URL": tt_url, "TT Views": tt_views or "", "TT Likes": tt_likes or "",
                    "TT Comments": tt_comments or "",
                    "TT Shares": tt_shares or "", "TT Saves": tt_saves or "",
...
def edit_post_dialog(post_dict, campaign_id, campaign_name):
    ...
    with hcol2:
        del_top_key = f"ep_del_top_{post_sr}"
        if st.session_state.get(del_top_key):
            if st.button("⚠ Confirm", key=f"ep_delc_top_{post_sr}", use_container_width=True):
                try:
                    gc = get_gspread_client()
                    ws = gc.open_by_key(SHEET_ID).worksheet("Post Tracking")
                    ws.delete_rows(post_sr)
                    st.session_state[del_top_key] = False
                    st.session_state[f"cd_editmode_{campaign_id}"] = None
                    st.success("Deleted."); st.cache_data.clear(); st.rerun()
                except Exception as e: st.error(f"Delete failed: {e}")
        else:
            if st.button("🗑 Delete", key=f"ep_del_top_btn_{post_sr}", use_container_width=True):
                st.session_state[del_top_key] = True; st.rerun()
    ...
    c1, c2 = st.columns(2)
    with c1:
        existing_date_str = str(post_dict.get("Date", ""))[:10]
        try:
            initial_date = datetime.strptime(existing_date_str, "%Y-%m-%d").date()
        except Exception:
            initial_date = date.today()
        d_date_obj = st.date_input("Date posted", value=initial_date, key=f"ep_date_{post_sr}", format="YYYY-MM-DD")
        d_date = d_date_obj.strftime("%Y-%m-%d") if d_date_obj else ""
    with c2:
        d_title = st.text_input("Title / Caption", value=str(post_dict.get("Title/Caption", "")), key=f"ep_title_{post_sr}")
    st.markdown("<div style='display:flex;align-items:center;gap:8px;margin-top:14px;margin-bottom:6px'>"
                "<i class='ti ti-brand-instagram' style='color:#1877F2;font-size:22px'></i>"
                "<span style='font-size:13px;font-weight:600;color:#FAFAFA'>Instagram</span></div>", unsafe_allow_html=True)
    ig_url_existing = str(post_dict.get("IG URL", ""))
    ig_url = st.text_input("IG URL", value=ig_url_existing, key=f"ep_ig_url_{post_sr}", label_visibility="collapsed", placeholder="instagram.com/p/…")
    if ig_url_existing:
        st.markdown(f"<a href='{ig_url_existing}' target='_blank' style='font-size:11px;color:#1877F2;text-decoration:none;margin-bottom:8px;display:inline-block'>"
                    f"<i class='ti ti-external-link' style='font-size:11px;vertical-align:-1px'></i> Open in Instagram</a>", unsafe_allow_html=True)
    ic1, ic2, ic3 = st.columns(3)
    with ic1:
    ...
    st.markdown("<div style='display:flex;align-items:center;gap:8px;margin-top:14px;margin-bottom:6px'>"
                "<i class='ti ti-brand-youtube' style='color:#E24B4A;font-size:22px'></i>"
                "<span style='font-size:13px;font-weight:600;color:#FAFAFA'>YouTube</span></div>", unsafe_allow_html=True)
    yt_url_existing = str(post_dict.get("YT URL", ""))
    yt_url = st.text_input("YT URL", value=yt_url_existing, key=f"ep_yt_url_{post_sr}", label_visibility="collapsed", placeholder="youtu.be/… or youtube.com/watch?v=…")
    if yt_url_existing:
        st.markdown(f"<a href='{yt_url_existing}' target='_blank' style='font-size:11px;color:#E24B4A;text-decoration:none;margin-bottom:8px;display:inline-block'>"
                    f"<i class='ti ti-external-link' style='font-size:11px;vertical-align:-1px'></i> Open in YouTube</a>", unsafe_allow_html=True)
    yc1, yc2, yc3 = st.columns(3)
    ...
    with yc2: yt_likes = st.number_input("YT Likes", min_value=0, step=10, value=_safe_int(post_dict.get("YT Likes")), key=f"ep_yt_l_{post_sr}")
    with yc3: yt_comments = st.number_input("YT Comments", min_value=0, step=1, value=_safe_int(post_dict.get("YT Comments")), key=f"ep_yt_c_{post_sr}")
    st.markdown("<div style='display:flex;align-items:center;gap:8px;margin-top:14px;margin-bottom:6px'>"
                "<i class='ti ti-brand-tiktok' style='color:#FAFAFA;font-size:22px'></i>"
                "<span style='font-size:13px;font-weight:600;color:#FAFAFA'>TikTok</span></div>", unsafe_allow_html=True)
    tt_url_existing = str(post_dict.get("TT URL", ""))
    tt_url = st.text_input("TT URL", value=tt_url_existing, key=f"ep_tt_url_{post_sr}", label_visibility="collapsed", placeholder="tiktok.com/@…/video/…")
    if tt_url_existing:
        st.markdown(f"<a href='{tt_url_existing}' target='_blank' style='font-size:11px;color:#FAFAFA;text-decoration:none;margin-bottom:8px;display:inline-block'>"
                    f"<i class='ti ti-external-link' style='font-size:11px;vertical-align:-1px'></i> Open in TikTok</a>", unsafe_allow_html=True)
    tc1, tc2, tc3 = st.columns(3)
    ...
    tc4, tc5 = st.columns(2)
    with tc4: tt_shares = st.number_input("TT Shares", min_value=0, step=10, value=_safe_int(post_dict.get("TT Shares")), key=f"ep_tt_s_{post_sr}")
    with tc5: tt_saves = st.number_input("TT Saves", min_value=0, step=10, value=_safe_int(post_dict.get("TT Saves")), key=f"ep_tt_sv_{post_sr}")
    st.caption("IG Reel includes cross-posted FB stats. ER % = engagement ÷ views.")
...
def campaign_detail_dialog(camp_dict):
    ...
    camp_sr_top = int(camp_dict.get("_sheet_row", 0))
    edit_camp_key = f"cd_editcamp_{cid}"

    # Edit campaign mode — render edit form and return
    if st.session_state.get(edit_camp_key):
        st.markdown("<p class='brand'>Priya Sid Enterprise</p>", unsafe_allow_html=True)
        st.markdown(f"<p class='app-name' style='margin-top:4px'>Edit campaign info</p>", unsafe_allow_html=True)
        if st.button("← Cancel", key=f"cd_editcamp_cancel_{cid}"):
            st.session_state[edit_camp_key] = False
            st.rerun()
        with st.form(f"edit_camp_form_{cid}", clear_on_submit=False):
            new_name = st.text_input("Campaign Name", value=str(name), key=f"ec_name_{cid}")
            ecc1, ecc2 = st.columns(2)
            with ecc1:
                new_brand = st.text_input("Brand", value=str(camp_dict.get("Brand", "")), key=f"ec_brand_{cid}")
                month_opts = ["", "January","February","March","April","May","June",
                              "July","August","September","October","November","December",
                              "Jan-Feb 2026", "Apr-Jun 2026", "Oct-Dec", "TBD"]
                if month and month not in month_opts: month_opts.insert(0, month)
                new_month = st.selectbox("Month", month_opts, index=month_opts.index(month) if month in month_opts else 0, key=f"ec_month_{cid}")
                fy_opts = ["FY25","FY26","FY27"]
                if fy and fy not in fy_opts: fy_opts.insert(0, fy)
                new_fy = st.selectbox("FY", fy_opts, index=fy_opts.index(fy) if fy in fy_opts else 1, key=f"ec_fy_{cid}")
            with ecc2:
                new_parent = st.text_input("Parent/Group", value=str(deal_group), key=f"ec_pg_{cid}")
                region_opts = ["India","Australia","UAE","US/Global",""]
                if region and region not in region_opts: region_opts.insert(0, region)
                new_region = st.selectbox("Region", region_opts, index=region_opts.index(region) if region in region_opts else 0, key=f"ec_region_{cid}")
                cur_opts = ["", "INR","USD","AED","AUD"]
                if currency and currency not in cur_opts: cur_opts.insert(0, currency)
                new_currency = st.selectbox("Currency", cur_opts, index=cur_opts.index(currency) if currency in cur_opts else 0, key=f"ec_cur_{cid}")
            new_rate = st.number_input("Rate (orig)", min_value=0.0, step=1000.0, value=float(rate_orig or 0), key=f"ec_rate_{cid}")
            new_status = st.text_input("Status", value=str(camp_dict.get("Status", "")), key=f"ec_status_{cid}")
            new_notes = st.text_area("Notes", value=str(camp_dict.get("Notes", "")), key=f"ec_notes_{cid}")
            ...
            if saved:
                try:
                    ...
                    headers = ws.row_values(1)
                    new_rate_display = format_rate_chip(new_currency, new_rate)
                    fields = {"Campaign Name": new_name, "Brand": new_brand, "Deal Group": new_parent,
                              "Month": new_month, "FY": new_fy, "Status": new_status,
                              "Region": new_region, "Currency": new_currency,
                              "Rate (orig)": new_rate, "Rate Display": new_rate_display,
                              "Notes": new_notes}
                    updates = []
                    for k, v in fields.items():
                        if k in headers:
                            col = headers.index(k) + 1
                            updates.append({"range": gspread.utils.rowcol_to_a1(camp_sr_top, col), "values": [[v]]})
                    if updates:
                        ws.batch_update(updates, value_input_option="USER_ENTERED")
                    st.session_state[edit_camp_key] = False
                    st.success("Saved!"); st.cache_data.clear(); st.rerun()
    ...
    with hh2:
        if st.button("✎ Edit info", key=f"cd_editcamp_btn_{cid}", use_container_width=True):
            st.session_state[edit_camp_key] = True
            st.rerun()
    if rate_chip:
    ...
    cpv_clr = cpv_color(agg["cpv"], currency, fx_rates)

    cd_tiles_html = (
        tile_grid_html([
            {"label": "Total views", "value": format_compact_int(agg["combined_views"])},
            {"label": "Total engagement", "value": format_compact_int(agg["combined_engagement"])},
        ])
        + tile_grid_html([
            {"label": "ER %", "value": er_str},
            {"label": f"CPV {currency}", "value": cpv_str, "color": cpv_clr},
        ])
        + tile_grid_html([
            {"label": f"CPM {currency}", "value": cpm_str},
            {"label": f"CPE {currency}", "value": cpe_str},
        ])
    )
    st.markdown(cd_tiles_html + _CPV_LEGEND_HTML, unsafe_allow_html=True)
    ...
    if st.session_state.get(add_mode_key):
        ...
        for i, p in enumerate(posts):
            ...
            st.markdown(f"<div class='deal-card card-overlay-trigger-lg' style='background:#18181B;border:1px solid #27272A;border-radius:10px;padding:10px;margin-bottom:0'>"
        ...
        with colA:
            if st.button("Confirm delete campaign", key=f"cd_campcdel_{camp_sr}", use_container_width=True):
                try:
                    ...
                    st.session_state["camp_detail_cid"] = None
...
with tabs[0]:
    ...
    ov_tiles_html = (
        tile_grid_html([
            {"label": "Gross Revenue", "value": format_money(gross_disp, display_cur),
             "subtitle": f"incl <span style='color:#10B981;font-weight:500'>+ {format_money(ad_disp, display_cur)}</span> Ad Revenue"},
            {"label": "Net Income", "value": format_money(net_disp, display_cur),
             "subtitle": "after Commission + TDS"},
        ])
        + tile_grid_html([
            {"label": "Avg Gross / Month", "value": format_money(avg_gross, display_cur),
             "subtitle": f"{elapsed}mo elapsed"},
            {"label": "Avg Net / Month", "value": format_money(avg_net, display_cur),
             "subtitle": f"{elapsed}mo elapsed"},
        ])
        + tile_grid_html([
            {"label": "Commission", "value": format_money(comm_disp, display_cur),
             "subtitle": "Agency 15%"},
            {"label": "TDS", "value": format_money(tds_disp, display_cur),
             "subtitle": "India 21.22%"},
        ])
    )
    st.markdown(ov_tiles_html, unsafe_allow_html=True)
...
with tabs[2]:
    ...
    def fmt_total_disp(amt):
        ...
        if display_cur == "AED":
            return f"د.إ {amt:,.0f}"
        return f"{cur_symbol}{amt:,.0f}"
    def build_monthly_region(region_filter):
        """region_filter: str (e.g. 'India') or None for all regions. Values in display currency."""
        m = {(y, mo): 0.0 for y, mo in months_list}
        c = 0
        for d in period_deals:
            if region_filter is not None and d.get("Region") != region_filter: continue
            if not d["month_date"]: continue
            key = (d["month_date"].year, d["month_date"].month)
            if key not in m: continue
            m[key] += d["gross_aud"] * mult
            c += 1
        return m, c
    def render_monthly_chart(title, monthly_dict, count, color):
        total = sum(monthly_dict.values())
        avg = total / elapsed if elapsed else 0
        st.markdown(
            f"<div style='display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin:14px 0 6px 0'>"
            f"<div style='flex:1;min-width:0'>"
            f"<p class='section-label' style='margin:0 0 2px 0'>{title}</p>"
            f"<p style='font-size:14px;font-weight:600;color:#FAFAFA;margin:0'>"
            f"{fmt_total_disp(total)} <span style='font-size:10px;color:#71717A;font-weight:400'>"
            f"total · {count} deals</span></p>"
            f"</div>"
            f"<div style='font-size:11px;color:#A1A1AA;text-align:right;padding-top:14px;white-space:nowrap;flex-shrink:0'>"
            f"Avg {fmt_total_disp(avg)} / month</div>"
            f"</div>",
            unsafe_allow_html=True)
        df = label_months(monthly_dict)
        suffix = ""
        if display_cur == "INR":
    ...
    monthly_all, count_all = build_monthly_region(None)
    render_monthly_chart("Monthly Gross — Brand Deals (All Regions)", monthly_all, count_all, "#10B981")

    # === Chart 2: India ===
    monthly_in, count_in = build_monthly_region("India")
    render_monthly_chart("India", monthly_in, count_in, "#FB923C")

    # === Chart 3: UAE ===
    monthly_uae, count_uae = build_monthly_region("UAE")
    render_monthly_chart("UAE", monthly_uae, count_uae, "#7F77DD")

    # === Chart 4: USA / Global ===
    monthly_us, count_us = build_monthly_region("US/Global")
    render_monthly_chart("USA / Global", monthly_us, count_us, "#ED93B1")

    # === Chart 5: Gross Revenue by Region (overview) ===
    region_colors = {"India": "#FB923C", "UAE": "#7F77DD", "US/Global": "#ED93B1", "Australia": "#378ADD"}
    region_totals = defaultdict(float)
    region_counts = defaultdict(int)
    for d in period_deals:
        rgn = d.get("Region")
        if rgn not in region_colors: continue
        region_totals[rgn] += d["gross_aud"] * mult
        region_counts[rgn] += 1
    overall_total = sum(region_totals.values())
    overall_count = sum(region_counts.values())
    st.markdown(
        f"<div style='display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin:14px 0 6px 0'>"
        f"<div style='flex:1;min-width:0'>"
        f"<p class='section-label' style='margin:0 0 2px 0'>Gross Revenue by Region</p>"
        f"<p style='font-size:14px;font-weight:600;color:#FAFAFA;margin:0'>"
        f"{fmt_total_disp(overall_total)} <span style='font-size:10px;color:#71717A;font-weight:400'>"
        f"total · {overall_count} deals</span></p>"
        f"</div>"
        f"<div style='font-size:11px;color:#A1A1AA;text-align:right;padding-top:14px;white-space:nowrap;flex-shrink:0'>"
        f"{period_label()}</div>"
        f"</div>",
    ...
    if region_totals:
        region_rows = []
        for rgn, tot in region_totals.items():
            v = tot / 100000 if display_cur == "INR" else tot
            region_rows.append({"Region": rgn, "Value": v})
    ...
    if display_cur == "INR":
        top10 = [(b, v/100000) for b, v in top10]
    df_top = pd.DataFrame(top10, columns=["Brand", "Value"])
    if len(df_top) > 0:
        suffix_h = "L" if display_cur == "INR" else ""
        st.plotly_chart(
...
with tabs[4]:
    # Page-style routing within the Campaigns tab via session state
    open_cid = st.session_state.get("camp_detail_cid")
    if open_cid:
        # CAMPAIGN DETAIL VIEW (page, not dialog)
        camp_list_lookup = process_campaigns_full(campaigns_raw, posts_raw) if not campaigns_raw.empty else []
        target = next((item for item in camp_list_lookup if item["camp"].get("Campaign ID") == open_cid), None)
        if target:
            if st.button("← Back to Campaigns", key="cd_back_to_list", use_container_width=False):
                st.session_state["camp_detail_cid"] = None
                # clean up sub-mode keys
                for k in [f"cd_addmode_{open_cid}", f"cd_editmode_{open_cid}"]:
                    if k in st.session_state: del st.session_state[k]
                st.rerun()
            campaign_detail_dialog(target["camp"])
        else:
            st.session_state["camp_detail_cid"] = None
            st.rerun()
    elif campaigns_raw.empty:
        hcol1, hcol2 = st.columns([3, 1])
        with hcol1:
        ...
        with fc2:
            ...
            reg_f = st.multiselect("Region", regions_c,
                                     default=["India"] if "India" in regions_c else [],
                                     key="camp_region_filter")
        ...
        for item in filtered_c:
            ...
            st.markdown(f"<div class='deal-card card-overlay-trigger-lg' style='{border_left}'>"
            ...
            if st.button("⠀", key=f"camp_btn_{cid}_{c.get('_sheet_row', 0)}", use_container_width=True):
                st.session_state["camp_detail_cid"] = cid
                st.rerun()
...
with tabs[5]:
    ...
    if campaigns_raw.empty:
        ...
        with ac4:
            ...
            an_region = st.multiselect("Region", regions_a,
                                         default=["India"] if "India" in regions_a else [],
                                         key="an_region_filter")
        filt = all_camps
        ...
        def render_aggregate_block(camps_in_currency, currency_label):
            ...
            avg_cpv_clr = cpv_color(avg_cpv, currency_label, fx_rates)
            med_cpv_clr = cpv_color(med_cpv, currency_label, fx_rates)
            st.markdown(f"<p class='section-label'>{currency_label} · {len(camps_in_currency)} campaigns</p>", unsafe_allow_html=True)
            block_html = (
                tile_grid_html([
                    {"label": "Avg CPV", "value": format_cpv(avg_cpv, currency_label), "color": avg_cpv_clr},
                    {"label": "Avg CPE", "value": format_cpv(avg_cpe, currency_label)},
                ])
                + tile_grid_html([
                    {"label": "Avg ER %", "value": f"{avg_er:.2f}%" if avg_er else "—"},
                    {"label": "Median CPV", "value": format_cpv(med_cpv, currency_label), "color": med_cpv_clr},
                ])
                + tile_grid_html([
                    {"label": "Median CPE", "value": format_cpv(med_cpe, currency_label)},
                    {"label": "Median ER %", "value": f"{med_er:.2f}%" if med_er else "—"},
                ])
            )
        ...
        if currencies_in_view:
            ...
            if with_perf:
                chart_sym = _currency_symbol(chart_currency)
                ...
                month_buckets = {m: [] for m in FY_MONTH_ORDER}
                for c in with_perf:
                    m = c["camp"].get("Month", "")
                ...
                if trend_df["Value"].sum() > 0:
                    fig = go.Figure(data=[go.Scatter(x=trend_df["Month"], y=trend_df["Value"],
                        ...
                        hovertemplate=f"<b>%{{x}}</b><br>{trend_metric}: {chart_sym}%{{y:.4f}}<extra></extra>")])
                    layout = base_layout(height=200)
                    fig.update_layout(**layout)
                    fig.update_yaxes(tickprefix=chart_sym, tickformat=".4f")
                ...
                if bot5:
                    ...
                    st.plotly_chart(make_hbar_chart(bot_df, "CPV", "Name", color="#E24B4A", currency_symbol=chart_sym, precision=4),
                                    use_container_width=True, config={"displayModeBar": False})
